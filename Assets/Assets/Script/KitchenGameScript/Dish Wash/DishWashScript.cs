...
public class DishWashScript : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (spongeRayCast.isHit && (spongeRayCast.hitObject == this.gameObject))
        {
            ...
            Vector2 textureCoords = spongeRayCast.hitOut.textureCoord;
            Debug.Log(textureCoords);
            int pixelX = Mathf.FloorToInt(textureCoords.x * _templateDirtMask.width);
            int pixelY = Mathf.FloorToInt(textureCoords.y * _templateDirtMask.height);
